...
Material* SceneParser::parseMaterial() {
  char token[MAX_PARSER_TOKEN_LENGTH];
  Vec3f diffuseColor(1.0f,1.0f,1.0f);
  Vec3f specularColor(0.0f,0.0f,0.0f);
  Vec3f transparentColor(0.0f,0.0f,0.0f);
  Vec3f reflectiveColor(0.0f,0.0f,0.0f);
  int exponent = 1;
  float ir = 1;
  getToken(token); assert (!strcmp(token, "{"));
  while (1) {
  ...
  Material *answer = new Material(diffuseColor, specularColor, exponent, reflectiveColor, transparentColor, ir);
  return answer;
}
